...
export class InvokeOpStore {
  ...
  @action
  loadCurrentSysPath = async (sysId) => {
    let json=await get(`${baseUrl}/sys/currentSysPath/${sysId}`);
    runInAction(()=>{
      this.currentSysPath=json[0];
    });
  };
  ...
  synInterfaces = async (sysId) => {
    ...
    if(!this.currentSysPath&& sysId !== 1){
      notification.warning({
        message: '该系统平台在平台功能配置中未配置服务资源目录',
      });
    }else{
